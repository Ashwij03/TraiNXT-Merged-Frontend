...
function SubjectAuditTrail() {
  ...
  const logs = (JSON.parse(localStorage.getItem("auditLogs")) || []).filter(
    (log) => String(log.subjectId) === String(subject?.id),
  );
    ...
    <div className="subject-tab-card tnxt-compact">
      ...
      <table className="ctms-standard-table">
        ...
        <tbody>
          {logs.length === 0 ? (
            <tr>
              <td colSpan="4" className="no-audit-data">
            ...
            logs.map((log, index) => (
              <tr key={index}>
                ...
                <td>
                  {log.updatedAt || log.deletedAt || log.createdAt
                    ? new Date(
                        log.updatedAt || log.deletedAt || log.createdAt,
                      ).toLocaleString()
                    : "-"}
