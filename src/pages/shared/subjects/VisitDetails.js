...
function VisitDetails({ setActiveTab }) {
  const visit = JSON.parse(localStorage.getItem("selectedVisit")) || {};
  ...
  return (
    <div className="visit-details-page tnxt-compact">
      <div className="visit-header">
        <button className="back-btn" onClick={() => setActiveTab("Visits")}>
          ← Back To Visits
        </button>

        <div className="visit-status">{visit.status || "Scheduled"}</div>
      </div>

      <div className="visit-title">
        <h1>{visit.name || "Visit"}</h1>
