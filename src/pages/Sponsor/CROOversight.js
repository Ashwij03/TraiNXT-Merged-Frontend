...
const CROOversight = () => {
  ...
  const exportData = () => {
    ...
    const csv =
      "CRO,Studies,Sites,Performance\n" +
      cros
        .map((c) => `${c.name},${c.studies},${c.sites},${c.performance}%`)
        .join("\n");
    ...
    <AppLayout>
      ...
      {viewCro && (
        <div
          ...
          onClick={() => setViewCro(null)}
          role="presentation"
        >
          <div className="sponsor-modal" onClick={(e) => e.stopPropagation()}>
            <h3>{viewCro.name}</h3>
            <p>
              <strong>Studies:</strong> {viewCro.studies}
            </p>
            <p>
              <strong>Sites:</strong> {viewCro.sites}
            </p>
            <p>
              <strong>Performance:</strong> {viewCro.performance}%
            </p>
            <p>
              <strong>Contact:</strong> {viewCro.contact}
            </p>
            <button type="button" onClick={() => setViewCro(null)}>
              Close
            </button>
          </div>
        </div>
