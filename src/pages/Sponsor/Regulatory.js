    ...
    <AppLayout>
      <div className="regulatory-page tnxt-compact">
        <div className="sponsor-page-header">
          <h1>Regulatory</h1>
          <p>
            Track regulatory submissions, approvals, and compliance documents.
          </p>
        </div>
              ...
              {filteredDocuments.length === 0 ? (
                ...
                filteredDocuments.map((doc) => (
                  <tr
                    key={doc.id}
                    onClick={() =>
                      navigate("/regulatory-details", { state: doc })
                    }
                  >
                    <td>{doc.id}</td>
                    <td>{doc.study}</td>
                    ...
                    <td>
                      <span
                        className={`status-badge ${doc.status === "Overdue" ? "open" : "active"}`}
                      >
                        {doc.status}
                      </span>
                    </td>
