    ...
    <AppLayout>
      <div className="reports-page">
        ...
        <div className="sponsor-kpi-grid">
          ...
          <KpiCard title="Templates" value={reportTemplates.length + documentReportCards.length} subtitle="Report categories" icon={<FiDownload size={24} />} iconBg="#ede9fe" iconColor="#7c3aed" />
        </div>
        ...
        <div className="reports-grid">
          {documentReportCards.map((report) => (
            <div key={report.title} className="report-card report-card--compliance" onClick={() => openReport(report.type)} role="button" tabIndex={0}>
              <h3>{report.title}</h3>
              <p>{report.description}</p>
              <button type="button" className="open-report-btn" onClick={(event) => { event.stopPropagation(); openReport(report.type); }}>
                Open Report
              </button>
            </div>
          ))}
        ...
        <div className="reports-grid">
          {reportTemplates.map((report) => (
            <div key={report.title} className="report-card" onClick={() => openReport(report.title)} role="button" tabIndex={0}>
              <h3>{report.title}</h3>
              <p>{report.description}</p>
              <button type="button" className="open-report-btn" onClick={(event) => { event.stopPropagation(); openReport(report.title); }}>
                Open Report
              </button>
            </div>
          ))}
      ...
      {showCreate && (
        <EnterpriseModal title="Generate Report" onClose={() => setShowCreate(false)} onSave={handleCreate} saveLabel="Generate">
          <input placeholder="Report Name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} />
          ...
          <input placeholder="Study (or All)" value={form.study} onChange={(event) => setForm({ ...form, study: event.target.value })} />
        </EnterpriseModal>
      )}
    </AppLayout>
  );
