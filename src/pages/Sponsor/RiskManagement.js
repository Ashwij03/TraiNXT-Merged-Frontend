...
const RiskManagement = () => {
  ...
  const filteredRisks = risks.filter((risk) => {
    ...
    if (statusFilter === "Resolved")
      return risk.status === "Closed" || risk.status === "Mitigated";
    if (statusFilter === "Critical/High")
      ...
      <div className="risk-page tnxt-compact">
        ...
        <div className="sponsor-kpi-grid">
          ...
          <KpiCard
            ...
            subtitle="Priority risks"
            icon={<FiAlertTriangle size={24} />}
            iconBg="#fee2e2"
            iconColor="#dc2626"
            onClick={() => setStatusFilter("Critical/High")}
          />
          <KpiCard
            title="Open Risks"
            value={kpis.open}
            ...
            <tbody>
              {filteredRisks.length === 0 ? (
                ...
                filteredRisks.map((risk) => (
                  <tr
                    key={risk.id}
                    onClick={() => navigate("/risk-details", { state: risk })}
                  >
                    <td>{risk.id}</td>
                    <td>{risk.title}</td>
                    <td>{risk.study}</td>
                    ...
                    <td>
                      <span
                        className={`status-badge ${risk.status === "Open" ? "open" : "active"}`}
                      >
                        {risk.status}
                      </span>
                    </td>
                    ...
                    <td onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        className="view-btn"
                        onClick={() =>
                          navigate("/risk-details", { state: risk })
                        }
                      >
                        View
                      </button>
