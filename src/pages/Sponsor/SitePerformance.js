...
const SitePerformance = () => {
  ...
  const exportData = () => {
    const csv =
      "Site,Study,Enrolled,Target,Performance\n" +
      sites
        .map(
          (s) =>
            `${s.name},${s.study},${s.enrolled},${s.target},${s.performance}%`,
        )
        .join("\n");
    ...
    <AppLayout>
      <div className="site-performance-page tnxt-compact">
        ...
        <div className="sponsor-kpi-grid">
          ...
          <KpiCard
            title="Regions"
            value={new Set(sites.map((s) => s.region)).size}
                ...
                <tr key={site.id} onClick={() => setViewSite(site)}>
                  ...
                  <td onClick={(e) => e.stopPropagation()}>
                    <button
                      type="button"
                      className="view-btn"
                      onClick={() => navigate("/site-details", { state: site })}
                    >
                      View
                    </button>
                  </td>
      ...
      {viewSite && (
        <EnterpriseModal
          title={formatSiteOption(viewSite) || MISSING_SITE_DISPLAY}
          onClose={() => setViewSite(null)}
        >
          <p>
            <strong>Site ID:</strong> {viewSite.id}
          </p>
          <p>
            <strong>Study:</strong> {viewSite.study}
          </p>
          <p>
            <strong>Enrollment:</strong> {viewSite.enrolled} / {viewSite.target}
          </p>
          <p>
            <strong>Performance:</strong> {viewSite.performance}%
          </p>
          <p>
            <strong>Region:</strong> {viewSite.region}
          </p>
        </EnterpriseModal>
      )}
    </AppLayout>
