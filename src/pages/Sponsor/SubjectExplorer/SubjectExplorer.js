 ...
 * Subject" action in the sidebar opens CreateSubjectModal (root-level,
 * mirrors CreateFolderModal); each subject row's context menu now also
 * exposes "Edit Subject" / "Delete Subject" (EditSubjectModal /
...
function SubjectExplorer({
  ...
  const [tree, setTree] = useState(() =>
    FolderTreeService.loadFolderTree(seedTree)
  );

  const [expandedIds, setExpandedIds] = useState(["SUB-001"]);
  ...
  useEffect(
    () =>
      FolderTreeService.subscribeFolderTree(() => {
        setTree(FolderTreeService.loadFolderTree(seedTree));
      }),
    [seedTree]
  );
  ...
  const submitCreate = (name) => {
    ...
    const result = FolderTreeService.createFolder(tree, parentId, name);
  ...
  const submitRename = (name) => {
    const result = FolderTreeService.renameFolder(tree, dialog.node.id, name);
  ...
  const submitDelete = () => {
    ...
    const result = FolderTreeService.deleteFolder(tree, target.id);
